...
const form = useForm({
    ...
    meeting_link: '',
    date: '',
    start_time: { hours: 9, minutes: 0 }, // Default 9:00 AM
    end_time: { hours: 10, minutes: 0 }, // Default 10:00 AM
...
const toggleCategory = (id: number) => {
    if (isSelected(id)) {
        form.category_ids = form.category_ids.filter((itemId) => itemId !== id);
    } else {
        ...
        <div class="mx-auto max-w-6xl p-4 sm:p-8">
            ...
            <form @submit.prevent class="grid grid-cols-1 gap-6 md:grid-cols-2">
                ...
                >
                    ...
                    >
                        {{ form.processing ? 'Publishing...' : 'Create Event' }}
                    </button>
                </div>
            </form>
        </div>
