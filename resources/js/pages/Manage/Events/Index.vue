...
const handleSearch = () => {
    router.get(
        '/manage/events',
        { search: searchQuery.value, sort: props.filters.sort || 'upcoming' },
        { preserveState: true },
    );
};
    ...
    <DashboardLayout :breadcrumbs="breadcrumbItems">
        ...
        <Teleport to="body">
            <CustomDialog
                ...
                :title="'Delete the event ' + selectedEvent?.title + '?'"
                description="By deleting the event, it will be removed from the system permanently."
